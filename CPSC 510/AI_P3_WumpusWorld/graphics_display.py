...
GHOST_COLORS.append(format_color(.1,.9,.1)) # Green
...
# Food
FOOD_COLOR = format_color(1,0.9,0)
FOOD_SIZE = 0.2
...
# Capsule graphics
PIT_COLOR = format_color(1,0.1,0.1)
PIT_SIZE = 0.5
...
class InfoPane:
    ...
    def to_screen(self, pos, y = None):
        ...
        x = max(self.grid_size/10,3) + x # Margin
        y = self.base + y
        return x,y

    def draw_pane(self):
        self.score_text = text_on_canvas( self.to_screen(0, 0  ), self.textColor, "SCORE:    0   Sensors: []", "Times", self.fontSize, "bold")
    ...
    def initialize_ghost_distances(self, distances):
        ...
        for i, d in enumerate(distances):
            t = text_on_canvas( self.to_screen(self.width/2 + self.width/8 * i, 0), GHOST_COLORS[i+1], d, "Times", size, "bold")
            self.ghost_distance_text.append(t)

    def update_score(self, score, sensors):
        change_text(self.score_text, "SCORE: % 4d  Sensors: %s" % (score, str(sensors)))

    def set_team(self, is_blue):
        text = "RED TEAM"
        if is_blue: text = "BLUE TEAM"
        self.team_text = text_on_canvas( self.to_screen(300, 0  ), self.textColor, text, "Times", self.fontSize, "bold")
...
class WumpusGraphics:
    def __init__(self, zoom=1.0, frameTime=0.0, capture=False, show_wumpus_world=True):
        print("Initialize WumpusGraphics")
        self.have_window = 0
        ...
        self.show_wumpus_world = show_wumpus_world
    ...
    def pit_images(self, agent_index, new_state):
        """
          Changes an image from a ghost to a pacman or vis versa (for capture)
        """
        prevState, prevImage = self.agent_images[agent_index]
        ...
        #if new_state.is_pacman:
        #    image = self.draw_capsules(new_state, agent_index)
        #    self.agent_images[agent_index] = (new_state, image )
        refresh()
    ...
    def update(self, new_state):
        ...
        #print "Agent update: index=",agent_index,"  state=",agent_state
        #print "images=",self.agent_images

        if (agent_index > 0 and not self.show_wumpus_world):
            #print "don't update wumpus if dark"
            return
        ...
        if agent_state.is_pacman:
            self.animate_pacman(agent_state, prevState, prevImage)
        else:
            self.move_ghost(agent_state, agent_index, prevState, prevImage)
        self.agent_images[agent_index] = (agent_state, prevImage)

        if (self.show_wumpus_world):
            if new_state._food_eaten != None:
                self.remove_food(new_state._food_eaten, self.food)
            if new_state._capsule_eaten != None:
                self.agent_images[agent_index] = None
                refresh()
        else:
            if new_state._capsule_eaten != None:
                self.agent_images[agent_index] = None # Need to get rid of Pacman
                refresh()

        self.info_pane.update_score(new_state.score,new_state.agent_states[0].get_sensors(new_state))
    ...
    def make_window(self, width, height):
        ...
        begin_graphics(screen_width,
                       screen_height,
                       BACKGROUND_COLOR,
                       "CPSC 471/510 Pacman Wumpus World")
    ...
    def animate_pacman(self, pacman, prevPacman, image):
        if self.frameTime < 0:
    ...
    def draw_ghost(self, ghost, agent_index):
        ...
        ghost_image_parts.append(leftEye)
        ghost_image_parts.append(rightEye)
        ghost_image_parts.append(leftPupil)
        ghost_image_parts.append(rightPupil)
    ...
    def move_ghost(self, ghost, ghostIndex, prevGhost, ghost_image_parts):
    ...
    def draw_capsules(self, capsules ):
        ...
        for capsule in capsules:
            ...
            dot = circle( (screen_x, screen_y),
                              PIT_SIZE * self.grid_size,
                              outline_color = PIT_COLOR,
                              fill_color = PIT_COLOR,
                              width = 1)
    ...
    def removeCapsule(self, cell, capsule_images ):
        x, y = cell
...
class FirstPersonPacmanGraphics(WumpusGraphics):
    def __init__(self, zoom = 1.0, showGhosts = True, capture = False, frameTime=0):
        print("Initialize FirstPersonPacmanGraphics ...")
