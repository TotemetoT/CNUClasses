...
def change_text(text_id, new_text, font=None, size=12, style='normal'):
    __canvas__.itemconfigure(text_id, text=new_text)
    if font is not None:
        __canvas__.itemconfigure(text_id, font=(font, '-%d' % size, style))


def change_color(text_id, new_color):
    __canvas__.itemconfigure(text_id, fill=new_color)
...
def move_by(obj, x, y=None,
    ...
    horiz = True
    new_coords = []
    for coord in __canvas__.coords(obj):
        if horiz:
            inc = x
        else:
            inc = y
    ...
    __canvas__.coords(obj, *new_coords)
    d_o_e(d_w)
    if lift:
        __canvas__.tag_raise(obj)
