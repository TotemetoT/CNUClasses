...
class AgentState:
    ...
    def copy(self):
        try:
            ...
            self.is_pacman = self.isPacman
            state.configuration = self.configuration
            state.scared_timer = self.scaredTimer
            self.scared_timer = self.scaredTimer
            state.num_carrying = self.numCarrying
            self.num_carrying = self.numCarrying
            state.num_returned = self.numReturned
            self.num_returned = self.numReturned
...
class Actions:
    ...
    """
    # todo make the equality function like in py2 or redo solution file
    # Directions
    ...
    # _directions_as_list = list(_directions)
    # In conversion from py2 to py3 the way the list cast sorted the resulting list was changed,
    # thus we just directly set the list so that we end up with the right order for the autograder.

    _directions_as_list = [('West', (-1, 0)),
                           ('Stop', (0, 0)),
                           ('East', (1, 0)),
                           ('North', (0, 1)),
                           ('South', (0, -1))]
    ...
    def get_possible_actions(config, walls):
        ...
        for my_dir, vec in Actions._directions_as_list:
            dx, dy = vec
            next_y = y_int + dy
            next_x = x_int + dx

            if not walls[next_x][next_y]:
                possible.append(my_dir)
    ...
    def get_legal_neighbors(position, walls):
        ...
        x_int, y_int = int(x + 0.5), int(y + 0.5)
        neighbors = []
        for _, vec in Actions._directions_as_list:
            dx, dy = vec
            next_x = x_int + dx
...
class GameStateData:
    ...
    def __init__(self, prev_state=None):
        ...
        if prev_state is not None:
            ...
            try:
                ...
                self.agent_states = self.copy_agent_states(prev_state.agentStates)
                self.eaten = prev_state._eaten
    ...
    def __hash__(self):
        ...
        try:
            ...
            self.agent_states = self.agentStates
        for i, state in enumerate(self.agent_states):
            try:
                ...
                print(e)
                # hash(state)
        return int((hash(tuple(self.agent_states)) + 13 * hash(self.food) + 113 * hash(tuple(
            self.capsules)) + 7 * hash(self.score)) % 1048575)
    ...
    def __str__(self):
        ...
        for agent_state in self.agent_states:
            ...
            if agent_state.is_pacman:
                my_map[x][y] = self._pac_str(agent_dir)
            else:
                my_map[x][y] = self._ghost_str(agent_dir)
    ...
    @staticmethod
    def _pac_str(my_dir):
        if my_dir == Directions.NORTH:
            return 'v'
        if my_dir == Directions.SOUTH:
            return '^'
        if my_dir == Directions.WEST:
            return '>'
        return '<'

    @staticmethod
    def _ghost_str(my_dir):
        return 'G'
        # code was here but was unreachable
        # if my_dir == Directions.NORTH:
        #     return 'M'
        # if my_dir == Directions.SOUTH:
        #     return 'W'
...
class Game:
    ...
    def __init__(self, agents, display, rules, starting_index=0, mute_agents=False,
    ...
    def _agent_crash(self, agent_index, quiet=False):
        ...
        self.rules.agent_crash(agent_index)
        ...
        while not self.game_over:
            ...
            if self.catch_exceptions:
                try:
                    ...
                    #                              - int(move_time))
                    timed_func = TimeoutFunction(agent.get_action,
                                                 int(self.rules.get_move_timeout()) - int(
                                                     move_time))
                    try:
                        start_time = time.time()
                        if skip_action:
