            ...
            for iteration in range(self.max_iterations):
                for i in enumerate(training_data):
                    ...
                    if predicted_label != training_labels[i]:
                        ...
                        numerator = (self.weights[predicted_label] - self.weights[true_label]) * f + 1
                        denominator = 2.0 * (f*f)
                        ...
                        scaled_features = util.Counter()
                        for feat, value in f.items():
                            scaled_features[feat] = value * tau

                        self.weights[true_label] += scaled_features
                        self.weights[predicted_label] -= scaled_features
