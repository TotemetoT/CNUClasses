...
def enhanced_feature_extractor_digit(datum):
    ...
    for x in range(width):
        for y in range(height):
            features[(x, y)] = 1 if pixels[x][y] > 0 else 0
    ...
    for x in range(width):
        for y in range(height):
            if pixels[x][y] == 0 and (x, y) not in visited:
                if not flood(x, y):  # white region that doesn't touch edge
                    hole_count += 1

    features["has-hole"] = 1 if hole_count > 0 else 0
...
def enhanced_pacman_features(state, action):
    ...
    pacman_pos = successor.get_pacman_position()

    #Food
    food = successor.get_food()
    food_list = food.as_list()
    if food_list:
        closest_food = min(manhattan_distance(pacman_pos, f) for f in food_list)
        features["food"] = 1.0 / (1.0 + closest_food)

    #Ghosts
    ghosts = successor.get_ghost_states()
    ghost_positions = [g.get_position() for g in ghosts]
    if ghost_positions:
        closest_ghost = min(manhattan_distance(pacman_pos, g) for g in ghost_positions)
        features["danger"] = 1.0 / (1.0 + closest_ghost)
    return features
