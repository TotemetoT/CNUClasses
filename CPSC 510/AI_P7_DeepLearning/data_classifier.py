    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if datum.get_pixel(x, y) > 0:
                ...
                features[(x, y)] = 0
                list_empty_positions[(x, y)] = False

    if partial_grid_dfs(list_empty_positions, DIGIT_DATUM_WIDTH, DIGIT_DATUM_HEIGHT):
        features["CC"] = 1
    else:
        features["CC"] = 0
...
def enhanced_pacman_features(state, action):
    ...
    # https://github.com/hyzhangsf/cs188_proj5/blob/master/dataClassifier.py
    state = state.generate_successor(0, action)
    pac_location = state.get_pacman_position()

    features['num_food_left'] = state.get_num_food()

    food_locs = state.get_food().as_list()
    nearest_food_dist = 0
    if food_locs:
        nearest_food_dist = min([util.manhattan_distance(pac_location, food) for food in food_locs])
    nearest_food_dist = 10.0 / nearest_food_dist if nearest_food_dist else nearest_food_dist
    features['nearest_food_dist'] = nearest_food_dist

    ghost_locs = state.get_ghost_positions()
    nearest_ghost_dist = 0
    if ghost_locs:
        nearest_ghost_dist = min(util.manhattan_distance(loc, pac_location) for loc in ghost_locs)
    nearest_ghost_dist = 10.0 / nearest_ghost_dist if nearest_ghost_dist else nearest_ghost_dist
...
def read_command(argv):
    ...
    if options.classifier == "mostFrequent":
        ...
        if options.data != 'pacman':
            classifier = neural_net.NeuralNetClassifier(legal_labels, options.iterations)
        else:
            classifier = neural_net_pacman.NeuralNetClassifierPacman(legal_labels,
                                                                     options.iterations)
