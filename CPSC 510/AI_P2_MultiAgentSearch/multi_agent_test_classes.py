...
from ghost_agents import DirectionalGhost, RandomGhost  # RandomGhost is used just not explicitly
...
class MultiagentTreeState:
    ...
    def get_score(self):
        ...
        if self.state not in self.problem.evaluation:
            raise Exception('get_score() called on non-terminal state or before maximum depth '
                            'achieved.')
        return float(self.problem.evaluation[self.state])

    def get_legal_actions(self, _):
        if VERBOSE:
            print("get_legal_actions(%s) -> %s" % (self.state, self.problem.state_to_actions[
                self.state]))
...
class MultiagentTreeProblem:
    ...
    def reset(self):
        self.generated_states = {self.start_state.state}
...
def parse_tree_problem(test_dict):
    ...
    return MultiagentTreeProblem(num_agents, start_state, win_states, lose_states, successors,
                                 evaluation)
...
def run(lay, lay_name, pac, ghosts, disp, n_games=1, name='games'):
    ...
    games = pacman.run_games(lay, pac, ghosts, disp, n_games, False, catch_exceptions=True,
                             timeout=120)
    print('*** Finished running %s on' % name, lay_name,
          'after %d seconds.' % (time.time() - starttime))
    stats = {'time': time.time() - starttime, 'wins': [g.state.is_win() for g in games].count(True),
             'games': games,
...
class GradingAgent(Agent):
    def __init__(self, seed, student_agent, optimal_actions, alt_depth_actions,
        ...
        self.actions_consistent_with_alternative_depth = [True for _ in
                                                          range(len(alt_depth_actions[0]))]
        self.actions_consistent_with_partial_ply_bug = [True for _ in
                                                        range(len(partial_ply_bug_actions[0]))]
    ...
    def get_action(self, state):
        ...
        if not cur_right_states_explored and self.wrong_states_explored < 0:
            print(" cur_right_states_explored=", cur_right_states_explored)
            print(" wrong_states_explored = ", self.wrong_states_explored, " - set to 1")
            print(" self.step_count=", self.step_count)
            print(" optimal actions = ", len(optimal_actions), optimal_actions)
            print(" student action = ", student_action)
    ...
    def check_failure(self):
        ...
        if self.actions_consistent_with_partial_ply_bug.count(True) > 0:
            return -2

        if self.actions_consistent_with_alternative_depth.count(True) > 0:
            return -1
...
class PolyAgent(Agent):
    ...
    def select(self, my_list, indices):
        """
    ...
    def construct_our_pacs(self, multi_agents, keyword_dict):
        pacs_without_stop = [multi_agents.StaffMultiAgentSearchAgent(**keyword_dict) for _ in
                             range(3)]
    ...
    def get_action(self, state):
        ...
        for agent in self.solution_agents:
            optimal_action_lists.append(
                (agent.getBestPacmanActions(state)[0], len(GameState.get_and_reset_explored())))
        alternative_depth_lists = [agent.getBestPacmanActions(state)[0] for agent in
                                   self.alternative_depth_agents]
        partial_ply_bug_lists = [agent.getBestPacmanActions(state)[0] for agent in
                                 self.partial_ply_bug_agents]
        # record responses
...
class PacmanGameTreeTest(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        self.layout_name = self.test_dict['layoutName']
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        all_actions = [json.loads(x) for x in solution_dict['optimalActions'].split('\n')]
        alt_depth_actions = [json.loads(x) for x in solution_dict['altDepthActions'].split('\n')]
        partial_ply_bug_actions = [json.loads(x) for x in
                                   solution_dict['partialPlyBugActions'].split('\n')]
        # set up game state and play a game
        random.seed(self.seed)
        lay = layout.Layout([l.strip() for l in self.layout_text.split('\n')])
        pac = GradingAgent(self.seed, student_agent, all_actions, alt_depth_actions,
                           partial_ply_bug_actions)
        # check return codes and assign grades
        disp = self.question.get_display()
        stats = run(lay, self.layout_name, pac, [DirectionalGhost(i + 1) for i in range(2)], disp,
                    name=self.alg)
        if stats['timeouts'] > 0:
    ...
    def write_list(self, handle, name, a_list):
        handle.write('%s: """\n' % name)
        for item in a_list:
            handle.write('%s\n' % json.dumps(item))
        handle.write('"""\n')
    ...
    def write_solution(self, module_dict, file_path):
        ...
        run(lay, self.layout_name, pac, [DirectionalGhost(i + 1) for i in range(2)], disp,
            name=self.alg)
...
class EvalAgentTest(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        self.layout_name = test_dict['layoutName']
        self.agent_name = test_dict['agentName']
        self.ghosts = eval(test_dict['ghosts'])
        self.max_time = int(test_dict['maxTime'])
        self.seed = int(test_dict['randomSeed'])
        self.num_games = int(test_dict['numGames'])
        ...
        self.agent_args = test_dict.get('agentArgs', '')
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        random.seed(self.seed)
        games = pacman.run_games(lay, agent,
                                 self.ghosts, disp,
                                 self.num_games, False,
                                 catch_exceptions=True, timeout=self.max_time)
        ...
        results = [grade_threshold(average_score, self.score_minimum, self.score_thresholds,
                                   "average score"),
