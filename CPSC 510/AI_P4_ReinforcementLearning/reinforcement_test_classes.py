...
class ValueIterationTest(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        iterations = int(test_dict['valueIterations'])
        if 'noise' in test_dict:
            self.grid.set_noise(float(test_dict['noise']))
        if 'livingReward' in test_dict:
            self.grid.set_living_reward(float(test_dict['livingReward']))
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        for n in self.nums_iterations_for_display:
            ...
            test_pass, std_out_string, file_out_string = self.execute_n_iterations(grades,
                                                                                   module_dict,
                                                                                   solution_dict,
                                                                                   n,
                                                                                   check_policy)
    ...
    def execute_n_iterations(self, _, module_dict, solution_dict, n, check_policy):
        test_pass = True
    ...
    def write_solution(self, module_dict, file_path):
        with open(file_path, 'w') as handle:
            ...
            actions = []
            for n in self.nums_iterations_for_display:
                values_pretty, q_values_pretty, actions, policy_pretty = self.run_agent(
                    module_dict, n)
                handle.write(self.pretty_value_solution_string('values_k_%d' % n, values_pretty))
                for action in actions:
                    handle.write(
                        self.pretty_value_solution_string('q_values_k_%d_action_%s' % (n, action),
    ...
    def run_agent(self, module_dict, num_iterations):
        agent = module_dict['value_iteration_agents'].ValueIterationAgent(self.grid,
...
class ApproximateQLearningTest(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        self.learning_rate = float(test_dict['learningRate'])
        self.extractor = 'IdentityExtractor'
        if 'extractor' in test_dict:
            self.extractor = test_dict['extractor']
        self.opts = {'actionFn': self.env.get_possible_actions, 'epsilon': self.epsilon,
                     'gamma': self.discount, 'alpha': self.learning_rate}
        num_experiences = int(test_dict['numExperiences'])
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        for n in self.nums_experiences_for_display:
            test_pass, std_out_string, file_out_string = self.execute_n_experiences(
                grades, module_dict, solution_dict, n)
            failure_output_std_string += std_out_string
            failure_output_file_string += file_out_string
            if not test_pass:
                self.add_message(failure_output_std_string)
    ...
    def execute_n_experiences(self, _, module_dict, solution_dict, n):
        test_pass = True
        ...
        if last_experience is not None:
            file_out_string += "Agent observed the transition " \
                               "(startState = %s, action = %s, endState = %s, reward = %f)\n\n" % \
                               last_experience
        ...
        for action in actions:
            ...
            if self.compare_pretty_values(q_values, solution_dict[q_values_key]):
                ...
                out_string = "Q-Values at iteration %d for action '%s' are NOT correct." % (
                    n, action)
                out_string += "   Student solution:\n\t%s" % self.pretty_value_solution_string(
                    q_values_key, q_values)
                out_string += "   Correct solution:\n\t%s" % self.pretty_value_solution_string(
                    q_values_key, solution_dict[q_values_key])
                std_out_string += out_string
    ...
    def write_solution(self, module_dict, file_path):
        with open(file_path, 'w') as handle:
            for n in self.nums_experiences_for_display:
                ...
                handle.write(
                    self.pretty_value_solution_string('weights_k_%d' % n, pp.pformat(weights)))
                for action in actions:
                    handle.write(
                        self.pretty_value_solution_string('q_values_k_%d_action_%s' % (n, action),
                                                          q_values_pretty[action]))
    ...
    def run_agent(self, module_dict, num_experiences):
        agent = module_dict['q_learning_agents'].ApproximateQAgent(extractor=self.extractor,
                                                                   **self.opts)
        states = list(filter(lambda state: len(self.grid.get_possible_actions(state)) > 0,
        ...
        for i in range(num_experiences):
            ...
            (end_state, reward) = self.env.get_random_next_state(start_state,
                                                                 action,
                                                                 rand_obj=rand_obj)
...
class QLearningTest(test_classes.TestCase):
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        for n in self.nums_experiences_for_display:
            ...
            test_pass, std_out_string, file_out_string = self.execute_n_experiences(
                grades, module_dict, solution_dict, n, check_values_and_policy)
            failure_output_std_string += std_out_string
            failure_output_file_string += file_out_string
            if not test_pass:
                self.add_message(failure_output_std_string)
    ...
    def execute_n_experiences(self, grades, module_dict, solution_dict, n, check_values_and_policy):
        test_pass = True
        values_pretty, q_values_pretty, actions, policy_pretty, last_experience = self.run_agent(
            module_dict, n)
        std_out_string = ''
        file_out_string = "==================== Iteration %d ====================\n" % n
        if last_experience is not None:
        ...
        if check_values_and_policy:
            if not self.compare_pretty_values(values_pretty, solution_dict['values']):
                ...
                out_string += "   Student solution:\n\t%s" % \
                              self.pretty_value_solution_string('values', values_pretty)
                out_string += "   Correct solution:\n\t%s" % \
                              self.pretty_value_solution_string('values', solution_dict['values'])
                std_out_string += out_string
                file_out_string += out_string
            if not self.compare_pretty_values(policy_pretty, solution_dict['policy']):
                test_pass = False
    ...
    def write_solution(self, module_dict, file_path):
        with open(file_path, 'w') as handle:
            ...
            for n in self.nums_experiences_for_display:
                ...
                for action in actions:
                    handle.write(
                        self.pretty_value_solution_string('q_values_k_%d_action_%s' %
    ...
    def run_agent(self, module_dict, num_experiences):
        ...
        for i in range(num_experiences):
            start_state = rand_obj.choice(states)
            action = rand_obj.choice(self.grid.get_possible_actions(start_state))
            end_state, reward = self.env.get_random_next_state(start_state, action,
                                                               rand_obj=rand_obj)
            last_experience = (start_state, action, end_state, reward)
            agent.update(*last_experience)
        actions = list(reduce(lambda a, b: set(a).union(b),
...
class EpsilonGreedyTest(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        self.epsilon = float(test_dict['epsilon'])
        self.learning_rate = float(test_dict['learningRate'])
        self.num_experiences = int(test_dict['numExperiences'])
    ...
    def test_epsilon_greedy(self, module_dict, tolerance=0.025):
        ...
        for state in self.grid.get_states():
            ...
            for iteration in range(self.num_iterations):
                # assume that their compute_action_from_q_values
                # implementation is correct (q4 tests this)
                if agent.get_action(state) == optimal_action:
                    num_greedy_choices += 1
            # e = epsilon, g = # greedy actions, n = numIterations, k = num_legal_actions
            # g = n * [(1-e) + e/k] -> e = (n - g) / (n - n/k)
            empirical_epsilon_numerator = self.num_iterations - num_greedy_choices
            empirical_epsilon_denominator = self.num_iterations - self.num_iterations / float(
                num_legal_actions)
            ...
            if error > tolerance:
                ...
                self.add_message("Actual epsilon = %f; student empirical epsilon = %f; "
                                 "error = %f > tolerance = %f"
                                 % (self.epsilon, empirical_epsilon, error, tolerance))
                return False
        return True
...
class EvalAgentTest(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        self.pacman_params = test_dict['pacmanParams']

        self.score_minimum = int(test_dict['scoreMinimum']) if 'scoreMinimum' in test_dict else None
        self.non_timeout_minimum = int(
            test_dict['nonTimeoutMinimum']) if 'nonTimeoutMinimum' in test_dict else None
        self.wins_minimum = int(test_dict['winsMinimum']) if 'winsMinimum' in test_dict else None

        self.score_thresholds = [int(s) for s in test_dict.get('scoreThresholds', '').split()]
        self.non_timeout_thresholds = [int(s) for s in
                                       test_dict.get('nonTimeoutThresholds', '').split()]
        self.wins_thresholds = [int(s) for s in test_dict.get('winsThresholds', '').split()]

        self.max_points = sum([len(t) for t in [self.score_thresholds, self.non_timeout_thresholds,
...
def compute_policy(module_dict, grid, discount):
    value_iterator = module_dict['value_iteration_agents'].ValueIterationAgent(grid,
                                                                               discount=discount)
    policy = {}
...
class GridPolicyTest(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        # Function in module in analysis that return_val (discount, noise)
        self.parameter_fn = test_dict['parameterFn']
        ...
        self.grid = grid_world.GridWorld(parse_grid(test_dict['grid']))
        self.grid_name = test_dict['gridName']
        ...
        self.path_visits = test_dict.get('pathVisits', None)

        # State the most probable path must not visit
        #    (x,y) for a particular location; (0,0) is bottom left
        #    terminal for the terminal state
        self.path_not_visits = test_dict.get('pathNotVisits', None)
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        if self.question2:
            ...
            if discount != 0.9 and noise != 0.2:
                self.add_message(
                    'Must change either the discount or the noise, not both. '
                    'Returned (discount, noise) = %s' % (result,))
                return self.test_fail(grades)
        else:
            try:
                ...
                self.add_message(
                    'Did not return a (discount, noise, living reward) triple; instead analysis.'
                    '%s returned: %s' % (self.parameter_fn, result))
                return self.test_fail(grades)
        ...
        for x in range(width):
            for y in range(height):
                if self.policy[x][y] in action_map and policy[(x, y)] != \
                        action_map[self.policy[x][y]]:
                    differ_point = (x, y)
                    policy_passed = False

        if not policy_passed:
            self.add_message('Policy not correct.')
            self.add_message('    Student policy at %s: %s' % (
                differ_point, policy[differ_point]))
            self.add_message('    Correct policy at %s: %s' % (
                differ_point, action_map[self.policy[differ_point[0]][differ_point[1]]]))
            ...
            self.add_message(
                "        Legend:  N,S,E,W for states in which the student "
                "policy must move north etc,")
            self.add_message(
                "                 _ for states where it doesn't matter what "
                "the student policy does.")
        ...
        if self.path_visits is not None and self.path_visits not in path:
            self.add_message(
                'Policy does not visit state %s when moving without noise.' % (self.path_visits,))
        ...
        if self.path_not_visits is not None and self.path_not_visits in path:
            self.add_message(
                'Policy visits state %s when moving without noise.' % (self.path_not_visits,))
            self.add_message('    States visited: %s' % (path,))
            self.add_message('    Student policy:')
            self.print_policy(policy, False)
            self.add_message(
