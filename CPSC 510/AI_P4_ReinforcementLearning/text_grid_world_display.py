...
def pretty_print_values(grid_world, values, policy=None, current_state=None):
    ...
    for row_num, row in enumerate(new_rows):
        row.insert(0, "\n\n" + str(row_num))
    new_rows.reverse()
    col_labels = [str(colNum) for colNum in range(num_cols)]
    col_labels.insert(0, ' ')
    final_rows = [col_labels] + new_rows
    print(indent(final_rows, separate_rows=True, delimiter='|', prefix='|',
                 postfix='|', justify='center', has_header=True))


def pretty_print_null_values(grid_world, current_state=None):
    grid_type = grid_world.grid
    max_len = 11
    new_rows = []
    for y in range(grid_type.height):
        new_row = []
        for x in range(grid_type.width):
            state = (x, y)
            ...
            if grid_type[x][y] == 'S':
                ...
                val_string = '\n#####\n#####\n#####\n'
                val_string += ' ' * max_len
            elif isinstance(grid_type[x][y], (float, int)):
                val_string = border('%.2f' % float(grid_type[x][y]))
            else:
                val_string = border('  ')
...
def pretty_print_q_values(grid_world, q_values, current_state=None):
    ...
    for y in range(grid_type.height):
        ...
        for x in range(grid_type.width):
            ...
            # display cell
            q_strings = {action: "%.2f" % q_values[(state, action)] for action in actions}
            north_string = q_strings['north'] if 'north' in q_strings else ' '
            south_string = q_strings['south'] if 'south' in q_strings else ' '
            east_string = q_strings['east'] if 'east' in q_strings else ' '
            west_string = q_strings['west'] if 'west' in q_strings else ' '
            exit_string = q_strings['exit'] if 'exit' in q_strings else ' '
...
def indent(rows, has_header=False, header_char='-', delimiter=' | ', justify='left',
           separate_rows=False, prefix='', postfix='', wrap_func=lambda x: x):
    """Indents a table by column.
       ...
       - header_char: Character to be used for the row separator line
         (if hasHeader==True or separate_rows==True).
       ...
       - wrap_func: A function f(text) for wrapping text; each element in
         the table is first wrapped by this function."""

    # closure for breaking logical rows to physical, using wrap_func
    def row_wrapper(row):
        new_rows = [wrap_func(item).split('\n') for item in row]
    ...
    row_separator = header_char * (len(prefix) + len(postfix) + sum(max_widths) +
                                   len(delimiter) * (len(max_widths) - 1))
    ...
    for physical_rows in logical_rows:
        for row in physical_rows:
            print(prefix + delimiter.join([justify(str(item), width) for item, width in zip(
                row, max_widths)]) + postfix, file=output)
...
if __name__ == '__main__':
    grid = grid_world.get_cliff_grid2()
    print(grid.get_states())

    policy = {state : 'east' for state in grid.get_states()}
    values = util.Counter({state : 1000.23 for state in grid.get_states()})
    pretty_print_values(grid, values, policy, current_state=(0, 0))

    state_cross_actions = [[(state, action) for action in grid.get_possible_actions(state)] for
                           state
                           in grid.get_states()]
    ...
    q_values = util.Counter({(state, action) : 10.5 for state, action in
                             reduce(lambda x, y: state_cross_actions, [])})
