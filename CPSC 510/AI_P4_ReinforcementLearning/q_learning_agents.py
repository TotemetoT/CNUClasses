...
class QLearningAgent(ReinforcementAgent):
    ...
    def compute_action_from_q_values(self, state):
        ...
        best_actions = [a for a in actions if self.get_q_value(state,a) == best_value]
    ...
    def update(self, state, action, next_state, reward):
        ...
        updated = (1 - self.alpha) * old_q + self.alpha * sample

        self.q_values[(state,action)] = updated
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, next_state, reward):
        ...
        if legal_actions:
            next_value = max(self.get_q_value(next_state, a) for a in legal_actions)
