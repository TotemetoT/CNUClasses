...
class InfoPane:
    ...
    def draw_pane(self):
        self.score_text = text_on_canvas(self.to_screen(0, 0), self.text_color,
                                         "SCORE:    0", "Times", self.font_size, "bold")
    ...
    def initialize_ghost_distances(self, distances):
        ...
        for i, d in enumerate(distances):
            t = text_on_canvas(self.to_screen(self.width / 2 + self.width / 8 * i, 0),
    ...
    def set_team(self, is_blue):
        ...
        if is_blue:
            team = "BLUE TEAM"
        self.team_text = text_on_canvas(self.to_screen(300, 0), self.text_color,
                                        team, "Times", self.font_size, "bold")

    def update_ghost_distances(self, distances):
        if not distances:
            return

        if self.ghost_distance_text is None:
...
class PacmanGraphics:
    ...
    def animate_pacman(self, pacman, prev_pacman, image):
        ...
        if self.frame_time > 0.01 or self.frame_time < 0:
            ...
            for i in range(1, int(frames) + 1):
                pos = px * i / frames + fx * (frames - i) / frames, py * i / \
    ...
    def draw_ghost(self, ghost, agent_index):
        ...
        for (x, y) in GHOST_SHAPE:
            coords.append((x * self.grid_size * GHOST_SIZE + screen_x, y
                           * self.grid_size * GHOST_SIZE + screen_y))
        ...
        for x_num, x in enumerate(wall_matrix):
            ...
            for y_num, cell in enumerate(x):
                if cell:  # There's a wall here
                    ...
                    # NE quadrant
                    if not (n_is_wall or e_is_wall):
                        # inner circle
                        circle(screen2, WALL_RADIUS * self.grid_size, wall_color, wall_color,
                               (0, 91), 'arc')
                    ...
                    if (not n_is_wall) and e_is_wall:
                        ...
                        line(add(screen, (0, self.grid_size * (-1) * WALL_RADIUS)),
                             add(screen,
                                 (self.grid_size * 0.5 + 1, self.grid_size * (-1) * WALL_RADIUS)),
                             wall_color)
                    if n_is_wall and e_is_wall and (not ne_is_wall):
                        # outer circle
                        circle(add(screen2, (
                            self.grid_size * 2 * WALL_RADIUS, self.grid_size * (-2) * WALL_RADIUS)),
                               WALL_RADIUS * self.grid_size - 1, wall_color, wall_color, (180, 271),
                               'arc')
                        line(add(screen, (
                            ...
                            self.grid_size * (-1) * WALL_RADIUS)),
                             ...
                             wall_color)
                        line(add(screen, (
                            self.grid_size * WALL_RADIUS, self.grid_size * (-2) * WALL_RADIUS + 1)),
                             add(screen, (self.grid_size * WALL_RADIUS, self.grid_size * (-0.5))),
                             wall_color)

                    # NW quadrant
                    if not (n_is_wall or w_is_wall):
                        # inner circle
                        circle(screen2, WALL_RADIUS * self.grid_size, wall_color, wall_color,
                               (90, 181), 'arc')
                    if n_is_wall and (not w_is_wall):
                        ...
                        line(add(screen, (self.grid_size * (-1) * WALL_RADIUS, 0)),
                             add(screen, (
                                 self.grid_size * (-1) * WALL_RADIUS, self.grid_size * (-0.5) - 1)),
                             wall_color)
                    ...
                    if n_is_wall and w_is_wall and (not nw_is_wall):
                        ...
                        circle(add(screen2, (
                            ...
                            self.grid_size * (-2) * WALL_RADIUS)),
                               WALL_RADIUS * self.grid_size - 1, wall_color, wall_color, (270, 361),
                               'arc')
                        line(
                        ...
                        line(
                            add(screen, (self.grid_size * (-1) * WALL_RADIUS,
                                         self.grid_size * (-2) * WALL_RADIUS + 1)),
                            add(screen,
                                (self.grid_size * (-1) * WALL_RADIUS, self.grid_size * (-0.5))),
                            wall_color)

                    # SE quadrant
                    if not (s_is_wall or e_is_wall):
                        # inner circle
                        circle(screen2, WALL_RADIUS * self.grid_size, wall_color, wall_color,
                               (270, 361), 'arc')
                    ...
                    if s_is_wall and e_is_wall and (not se_is_wall):
                        ...
                        circle(add(screen2, (
                            self.grid_size * 2 * WALL_RADIUS, self.grid_size * (2) * WALL_RADIUS)),
                               WALL_RADIUS * self.grid_size - 1, wall_color, wall_color, (90, 181),
                               'arc')
                        line(add(screen, (
                            self.grid_size * 2 * WALL_RADIUS - 1,
                            self.grid_size * (1) * WALL_RADIUS)),
                    ...
                    if s_is_wall and w_is_wall and (not sw_is_wall):
                        ...
                        circle(add(screen2, (
                            self.grid_size * (-2) * WALL_RADIUS,
                            self.grid_size * (2) * WALL_RADIUS)),
                               WALL_RADIUS * self.grid_size - 1, wall_color, wall_color, (0, 91),
                               'arc')
    ...
    def clear_expanded_cells(self):
        if 'expanded_cells' in dir(self) and self.expanded_cells:
