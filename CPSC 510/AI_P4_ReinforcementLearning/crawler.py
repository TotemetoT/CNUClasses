...
class CrawlingRobot:
    ...
    def move_arm(self, new_arm_angle):
        ...
        # self.angleSums.append(abs(math.degrees(old_arm_angle)-math.degrees(newArm_angle)))
    ...
    def move_hand(self, new_hand_angle):
        ...
        displacement = self.displacement(self.arm_angle, self.hand_angle, self.arm_angle,
                                         new_hand_angle)
        cur_x_pos = self.robot_pos[0]
        self.robot_pos = (cur_x_pos + displacement, self.robot_pos[1])
        self.hand_angle = new_hand_angle

        # Position and Velocity Sign Post
        self.positions.append(self.get_robot_position()[0])
        #       self.angleSums.append(abs(math.degrees(old_hand_angle)-math.degrees(newHand_angle)))
        if len(self.positions) > 100:
            self.positions.pop(0)
    ...
    def get_min_and_max_arm_angles(self):
        """
            ...
            for the arm angles return (min,max) pair
        """
    ...
    def get_min_and_max_hand_angles(self):
        """
            get the lower- and upper- bound
            for the hand angles return (min,max) pair
        """
    ...
    def __init__(self, canvas):
        ...
        self.hand_angle = self.old_hand_degree = -math.pi / 6

        self.max_arm_angle = math.pi / 6
        self.min_arm_angle = -math.pi / 6

        self.max_hand_angle = 0
        self.min_hand_angle = -(5.0 / 6.0) * math.pi
        ...
        self.ground = canvas.create_rectangle(0,
                                              self.ground_y, self.tot_width, self.tot_height,
                                              fill='blue')
