    ...
    def get_successors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nx][ny]:
                new_visited = set(visited_corners)
                if (nx,ny) in self.corners:
                    new_visited.add((nx,ny))
                successors.append((((nx,ny), tuple(new_visited)), action, 1))
...
def corners_heuristic(state, problem):
    ...
    #
    # queue = PriorityQueue()
    # queue.push((position, 0, unvisited.copy()), 0)
    #
    # while not queue.is_empty():
    #     cur_location, cur_cost, remaining = queue.pop()
    #     location = (cur_location, tuple(sorted(remaining)))
    ...
    #         nx, ny = x + dx, y + dy
    #         if not walls[nx][ny]:
    #             queue.push(((nx, ny), cur_cost + 1, remaining), cur_cost)
    #
    #
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def find_path_to_closest_dot(self, game_state):
        ...
        # Some useful elements of the start state
        ...
        if problem.is_goal_state(start_position):
            return []
        while not queue.is_empty():
            ...
            for next_state, direction, _ in problem.get_successors(state):
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def is_goal_state(self, state):
        ...
        x, y = state
        return (x, y) in self.food.as_list()
