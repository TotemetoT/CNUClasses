...
def follow_action(state, action, problem):
    for successor1, action1, _ in problem.get_successors(state):
        if action == action1:
            return successor1
    return None
...
class GraphSearch(SearchProblem):
    ...
    def __init__(self, graph_text):
        ...
        # todo investigae variable line which has no declaration
        if r is None:
            print("Broken graph:")
            print('"""%s"""' % graph_text)
            raise Exception("GraphSearch graph specification start_state "
                            "not found or incorrect on line:" + line)
        self.start_state = r.group(1).strip()
        r = re.match('goal_states:(.*)', lines[1])
        if r is None:
            print("Broken graph:")
            print('"""%s"""' % graph_text)
            raise Exception("GraphSearch graph specification goal_states "
                            "not found or incorrect on line:" + line)
    ...
    def __str__(self):
        ...
        return f'start_state: {self.start_state}\n' \
               f'goal_states: {" ".join(self.goals)}' + '%s' % ('\n'.join(edges))
...
class GraphSearchTest(test_classes.TestCase):
    ...
    def get_sol_info(self, search):
        ...
        if not isinstance(solution, list):
            return None, None, 'The result of %s must be a list. ' \
                               '(Instead, it is %s)' % (self.alg, type(solution))
...
class PacmanSearchTest(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        self.leeway_factor = float(test_dict.get('leewayFactor', '1'))
        self.cost_fn = eval(test_dict.get('costFn', 'None'))
        self.search_problem_class_name = test_dict.get('searchProblemClass',
    ...
    def get_sol_info(self, search, search_agents):
        ...
        heuristic = getattr(search_agents, self.heuristic_name) \
            if self.heuristic_name is not None else None
        ...
        if not isinstance(solution, list):
            return None, None, f'The result of {self.alg} must be a list. ' \
                               f'(Instead, it is {type(solution)})'

        dirs = list(Directions.LEFT.keys())
        if [el in dirs for el in solution].count(False) != 0:
            return None, None, f'Output of {self.alg} must be a list of actions ' \
                               f'from game.Directions'
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        if error is not None:
            grades.add_message(f'FAIL: {self.path}')
            grades.add_message(f'{error}')
            return False
        ...
        if solution not in gold_solution:
            ...
            grades.add_message(f'\tStudent solution length: {len(solution)}')
            grades.add_message(f'\tStudent solution:\n{wrap_solution(solution)}')
            grades.add_message('')
            grades.add_message(f'\tCorrect solution length: {len(gold_solution[0])}')
            grades.add_message(f'\tCorrect (reversed) solution length: {len(gold_solution[1])}')
            grades.add_message(f'\tCorrect (reversed) solution length: {len(gold_solution[1])}')
            grades.add_message(f'\tCorrect solution:\n{wrap_solution(gold_solution[0])}')
            grades.add_message(f'\tCorrect (reversed) solution:\n{wrap_solution(gold_solution[1])}')
            return False
        ...
        if expanded > self.leeway_factor * gold_expanded and expanded > gold_expanded + 1:
            ...
            grades.add_message(f'\tstudent nodes expanded: {expanded}')
            grades.add_message('')
            grades.add_message(f'\tcorrect nodes expanded: {gold_expanded} '
                               f'(leewayFactor {self.leeway_factor})')
            return False

        grades.add_message('PASS: %s' % self.path)
        grades.add_message(f'\tpacman layout:\t\t{self.layout_name}')
        grades.add_message(f'\tsolution length: {len(solution)}')
        grades.add_message(f'\tnodes expanded:\t\t{expanded}')
        return True
    ...
    def write_solution(self, module_dict, file_path):
        ...
        handle.write('# This solution is designed to support both right-to-left\n')
        handle.write('# and left-to-right implementations.\n')
        handle.write('# Number of nodes expanded must be with a factor '
                     'of %s of the numbers below.\n' % self.leeway_factor)
...
class CornerProblemTest(test_classes.TestCase):
    ...
    def solution(self, search, search_agents):
        ...
        missed_corners = [p for p in ((1, 1), (1, top),
                                      (right, 1), (right, top)) if p not in visited]
...
class HeuristicTest(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        self.layout_text = test_dict['layout']
        self.layout_name = test_dict['layoutName']
        self.search_problem_class_name = test_dict['searchProblemClass']
    ...
    @staticmethod
    def check_heuristic(heuristic, problem, state, solution_cost):
        ...
        for succ, _, step_cost in problem.get_successors(state):
...
class HeuristicGrade(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        self.search_problem_class_name = test_dict['searchProblemClass']
        self.heuristic_name = test_dict['heuristic']
        self.base_points = int(test_dict['basePoints'])
