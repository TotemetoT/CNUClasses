class AdvancedList(list):
    ...
    def __add__(self, other):
        ...
        if isinstance(other, list):
            if len(other) != len(self):
                raise ValueError("Lists must be of equal size for addition.")
            return AdvancedList([a + b for a, b in zip(self, other)])
        elif isinstance(other, (int, float)):
            return AdvancedList([a + other for a in self])
        else:
            raise TypeError("Operand must be a list, int, or float.")
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, list):
            if len(other) != len(self):
                raise ValueError("Lists must be of equal size for multiplication.")
            return AdvancedList([a * b for a, b in zip(self, other)])
        elif isinstance(other, (int, float)):
            return AdvancedList([a * other for a in self])
        else:
    ...
    def __lt__(self, other):
        ...
        if isinstance(other, list):
            if len(other) != len(self):
                raise ValueError("Lists must be of equal size for comparison.")
            return AdvancedList([a < b for a, b in zip(self, other)])
        elif isinstance(other, (int, float)):
            return AdvancedList([a < other for a in self])
        else:
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, slice):
            ...
            if not all(isinstance(i, bool) for i in index):
                raise TypeError("Boolean indexing list must contain only booleans.")
            return AdvancedList([item for item, mask in zip(self, index) if mask])
        else:
            raise TypeError("Index must be an int, slice, or list of booleans.")
