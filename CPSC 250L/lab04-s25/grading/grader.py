...
def grade_repo(args):
    ...
    from tests import test_add_book
    lab_score = score_unit_tests([test_add_book.TestAddBook])


    # Style checking
    if lab_score > 50.0:
        try:
            style_score = score_style(["src/add_book.py", "src/make_library.py"])
        except Exception as exc:
            print(exc, flush=True)
            style_score = 0.0
    else:
    ...
    final_grade = 0.30*style_score + 0.1*lab_score  # No separate HW grade for lab

    # For lab with 70 + 30 participation
    print(f"#Final grade = {final_grade:6.1f} of 40 + Participation + Demo (style={style_score:6.1f}, correctness={lab_score:6.1f})", flush=True)
...
if __name__ == "__main__":
    ...
    #print(sys.argv)
    if grade_repo(sys.argv[1:]) < 35: # For lab with 40 + 30 participation + 30 demo
